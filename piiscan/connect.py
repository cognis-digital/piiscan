"""Native cognis-connect emit for piiscan — forward findings to any platform.

Maps piiscan's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    piiscan ... --format json | piiscan-emit --to stix
    piiscan-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('column', '') + ' - ' + rec.get('top_entity', '')
        out['severity'] = 'medium'
        out['type'] = 'PII'
        out['description'] = f"Detected {rec.get('top_entity', '')} in column {rec.get('column', '')}"
        out['tags'] = [rec.get('entity_type', ''), rec.get('column', '')]
